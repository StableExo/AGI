// gemini-citadel/src/config/helpers/validators.ts
...
export function validateAndNormalizeAddress(rawAddress: string | undefined, contextName: string): string | null {
    ...
    try {
        ...
        logger.warn(`[Config Validate] ${contextName}: Validation error for "${rawAddress}" - ${(error as Error).message}`);
        return null;
    }
}
...
export function safeParseBigInt(valueStr: string | undefined, contextName: string, defaultValue = 0n): bigint {
    try {
        ...
        logger.warn(`[Config Parse BigInt] ${contextName}: Failed "${valueStr}": ${(e as Error).message}`);
        return defaultValue;
    }
}
