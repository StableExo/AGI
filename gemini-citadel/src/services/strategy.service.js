...
class StrategyEngine {
    constructor(dataProvider) {
        ...
        logger_service_1.default.info('[StrategyEngine] Initialized.');
    }
    /**
    ...
    async findOpportunities() {
        logger_service_1.default.info('[StrategyEngine] Analyzing markets for opportunities...');
        const opportunities = [];
        ...
        if (prices.length < 2) {
            logger_service_1.default.info('[StrategyEngine] Not enough pricing sources to find an arbitrage opportunity.');
            return [];
        }
    ...
    evaluateOpportunity(buySource, sellSource, pair, opportunities) {
        ...
        if (sellPrice > buyPrice) {
            ...
            logger_service_1.default.info(`[StrategyEngine] Evaluating: Buy on ${buySource.name} (${buyPrice}), Sell on ${sellSource.name} (${sellPrice}). Profit: ${estimatedProfit}`);
            if (estimatedProfit > (buyPrice * MIN_PROFIT_THRESHOLD)) {
                logger_service_1.default.info(`[StrategyEngine] Profitable opportunity found!`);
                const buyAction = {
                    action: 'BUY',
                ...
                const opportunity = new ArbitrageOpportunity_1.ArbitrageOpportunity(estimatedProfit, [buyAction, sellAction]);
